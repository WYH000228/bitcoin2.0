# BITCOIN2Sparknet Usage and Information

BITCOIN2is a decentralized blockchain project built on Bitcoin's UTXO model, but with support for Ethereum Virtual Machine based smart contracts. It achieves this through the revolutionary Account Abstraction Layer. For more general information about BITCOIN2as well as links to join our community, go to https://bitcoin2.org

Welcome to BITCOIN2Sparknet, the first public testnet for the BITCOIN2blockchain. Sparknet is designed primarily for developers, and as such documentation at this point will be technical and suited more for developers. The mainnet is expected to be released in September and will be suited for the general public. Testnet tokens do not hold any value and should not be traded for any monetary instruments. The testnet can be reset or forked at anytime as deemed necessary for development. Sparknet does not include support for Mutualized Proof Of Stake, or for the Decentralized Governance Protocol. Both of these features are implemented, and their code is available on alternative branches (check the pull requests), but have not been tested and proven stable enough to include in this testnet. They will be implemented in the 2nd public testnet for BTC2. 

# Using Smart Contracts with BTC2

The smart contract interface in BITCOIN2still requires some technical knowledge. The GUI is not completed yet, so all smart contract interation must happen either using `bitcoin2-cli` at the command line, or in the debug window of `bitcoin2-qt`. 

To demonstrate how to deploy and interact with a simple we will use this contract:

    pragma solidity ^0.4.0;

    contract BITCOIN2Test {
       uint storedNumber;
       function BITCOIN2Test() {
           storedNumber=1;
       }
...
(note, if using the debug window in the BITCOIN2Qt application, don't include `./bitcoin2-cli` in the commands)

First, we need to deploy the contract:

    ./bitcoin2-cli createcontract  300000
...
Now wait for your contract to be included in a block. You should be able to confirm it made it into a block by using:
    ...
    ./bitcoin2-cli getaccountinfo $CONTRACT
...
Now, because we are not changing state, we use `callcontract`:
    ...
    ./bitcoin2-cli callcontract $CONTRACT 3450bd6a
...
Note we use --lenient so that we do not need to provide a full 256bit value as a parameter. Now, we can execute the contract directly:
    ...
    ./bitcoin2-cli sendtocontract $CONTRACT 3fb5c1cb000000000000000000000000000000000000000000000000000000000001e240
...
This will send 10 tokens to the contract:
    ...
    ./bitcoin2-cli sendtocontract $CONTRACT d0e30db0 10

And then, to withdraw them it's also very simple:

    ./bitcoin2-cli sendtocontract $CONTRACT 3ccfd60b
...
If you want to control the exact address that the contract sends coins to, you can also explicitly specify the sender. Note that you must be capable of sending coins from that address (you can't use someone elses address). One of my wallet addresses is qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt, so I'll use that:
    ...
    ./bitcoin2-cli sendtocontract $CONTRACT 3ccfd60b 0 190000 0.0000001 qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt
...
Note that if you get the error "Sender address does not have any unspent outputs", then you should send some coins to that address (they must be spent in order to prove that you own that address). This can be accomplished with any amount of coins:
    ...
    ./bitcoin2-cli sendtoaddress qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt 0.001
...
* Q: "I think I found a bug in BTC2" A: Please report any bugs at https://github.com/cryptofugu/bitcoin2chain-core/issues



# New BITCOIN2RPC Commands

BITCOIN2supports all of the RPC commands supported by Bitcoin Core, but also includes the following commands unique to BTC2:

* `createcontract` - This will create and deploy a new smart contract to the BITCOIN2blockchain. This requires gas.
* `callcontract` - This will interact with an already deployed smart contract on the BITCOIN2blockchain, with all computation taking place off-chain and no persistence to the blockchain. This does not require gas
* `sendtocontract` - This will interact with an already deployed smart contract on the BITCOIN2blockchain. All computation takes place on-chain and any state changes will be persisted to the blockchain. This allows tokens to be sent to a smart contract. This requires gas.
...
# New BITCOIN2Command Line Arguments

BITCOIN2supports all of the usual command line arguments that Bitcoin Core supports. In addition it adds the following new command line arguments:

* `-record-log-opcodes` - This will create a new log file in the BITCOIN2data directory (usually ~/.bitcoin2) named vmExecLogs.json, where any EVM LOG opcode is logged along with topics and data that the contract requested be logged. 

# Untested features

Some features included in Bitcoin Core have not been tested in it's porting to BTC2. This includes:

* Pruning

# EVM Smart Contract Changes and Limitations

Because of BTC2's underlying technical differences, there are a few operations that can have different results or limitations when executed in BITCOIN2than when compared to Ethereum. 

These include the following, though there may be others introduced in the future:  

* The gas schedule for BITCOIN2is different from Ethereum. Certain operations are more or less expensive. As such, gas cost estimators designed for Ethereum will not give accurate results for BTC2. We will develop our own gas estimating tools as well as fully documenting these differences at a later date. 
...
Additional documents for the overall design and expected results of various operations is available at the ITD repository here: https://github.com/cryptofugu/bitcoin2-itds
