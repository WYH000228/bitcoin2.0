...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setBITCOIN2Schedule(bitcoin2DGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = bitcoin2DGP.getBlockSize(nHeight);
    minGasPrice = bitcoin2DGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = bitcoin2DGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    BITCOIN2TxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractBITCOIN2TX resultConverter;
    if(!convert.extractionBITCOIN2Transactions(resultConverter)){
    ...
    for(BITCOIN2Transaction bitcoin2Transaction : bitcoin2Transactions){
        txGas += bitcoin2Transaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + bitcoin2Transaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(bitcoin2Transaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, bitcoin2Transactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "bitcoin2stake";
...
void StakeBITCOIN2s(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
