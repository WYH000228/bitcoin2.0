...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // bitcoin2
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // bitcoin2
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // bitcoin2
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // bitcoin2
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination bitcoin2SenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(bitcoin2SenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&bitcoin2SenderAddress);
