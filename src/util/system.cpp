...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BTC2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BTC2
    // Mac: ~/Library/Application Support/BTC2
    // Unix: ~/.bitcoin2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTC2";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BTC2";
#else
    // Unix
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin 2.0 Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitcoin 2.0 Core Developers";
    }
