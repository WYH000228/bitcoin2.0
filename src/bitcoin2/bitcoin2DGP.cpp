#include <bitcoin2/bitcoin2DGP.h>
...
void BITCOIN2DGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool BITCOIN2DGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule BITCOIN2DGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t BITCOIN2DGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BITCOIN2DGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BITCOIN2DGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BITCOIN2DGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BITCOIN2DGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BITCOIN2DGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BITCOIN2DGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BITCOIN2DGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BITCOIN2DGP::createParamsInstance(){
...
dev::Address BITCOIN2DGP::getAddressForBlock(unsigned int blockHeight){
...
void BITCOIN2DGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BITCOIN2DGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BITCOIN2DGP::parseStorageOneUint64(uint64_t& value){
...
void BITCOIN2DGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BITCOIN2DGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
