...
#include <bitcoin2/bitcoin2state.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

BITCOIN2State::BITCOIN2State(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BITCOIN2State::openDB(_path + "/bitcoin2DB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BITCOIN2State::BITCOIN2State() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BITCOIN2State::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BITCOIN2Transaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBITCOIN2Address(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            bitcoin2::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> BITCOIN2State::vins() const // temp
{
...
void BITCOIN2State::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BITCOIN2State::vin(dev::Address const& _a) const
{
    return const_cast<BITCOIN2State*>(this)->vin(_a);
}

Vin* BITCOIN2State::vin(dev::Address const& _addr)
{
...
// void BITCOIN2State::commit(CommitBehaviour _commitBehaviour)
// {
...
void BITCOIN2State::kill(dev::Address _addr)
{
...
void BITCOIN2State::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void BITCOIN2State::deleteAccounts(std::set<dev::Address>& addrs){
...
void BITCOIN2State::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BITCOIN2State::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void BITCOIN2State::validateTransfersWithChangeLog(){
