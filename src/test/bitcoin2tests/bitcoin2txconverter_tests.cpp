...
void checkResult(bool isCreation, std::vector<BITCOIN2Transaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BITCOIN2TxConverter converter(transaction, NULL);
    ExtractBITCOIN2TX bitcoin2Tx;
    BOOST_CHECK(converter.extractionBITCOIN2Transactions(bitcoin2Tx));
    std::vector<BITCOIN2Transaction> result = bitcoin2Tx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(bitcoin2txconverter_tests, TestingSetup)
