...
#if defined(NDEBUG)
# error "BITCOIN2cannot be compiled without assertions."
#endif
...
std::unique_ptr<BITCOIN2State> globalState;
...
bool fAddressIndex = false; // bitcoin2
#endif
...
const std::string strMessageMagic = "BITCOIN2Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = bitcoin2DGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            BITCOIN2TxConverter converter(tx, NULL, NULL, contractflags);
            ExtractBITCOIN2TX resultConverter;
            if(!converter.extractionBITCOIN2Transactions(resultConverter)){
            ...
            std::vector<BITCOIN2Transaction> bitcoin2Transactions = resultConverter.first;
            std::vector<EthTransactionParams> bitcoin2ETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(BITCOIN2Transaction bitcoin2Transaction : bitcoin2Transactions){
                sumGas += bitcoin2Transaction.gas() * bitcoin2Transaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, bitcoin2Transaction.gasPrice());
                } else {
                    txMinGasPrice = bitcoin2Transaction.gasPrice();
                }
                VersionVM v = bitcoin2Transaction.getVersion();
                if(v.format!=0)
                ...
                if(bitcoin2Transaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(bitcoin2Transaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(bitcoin2Transaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += bitcoin2Transaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)bitcoin2Transaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(bitcoin2ETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > bitcoin2Transactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BITCOIN2additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // bitcoin2
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // bitcoin2
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    BITCOIN2Transaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<BITCOIN2Transaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path bitcoin2Dir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(bitcoin2Dir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(bitcoin2Dir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(BITCOIN2Transaction& tx : txs){
        //validate VM version
...
bool BITCOIN2TxConverter::extractionBITCOIN2Transactions(ExtractBITCOIN2TX& bitcoin2tx){
    ...
    std::vector<BITCOIN2Transaction> resultTX;
    ...
    bitcoin2tx = std::make_pair(resultTX, resultETP);
    return true;
}

bool BITCOIN2TxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool BITCOIN2TxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
BITCOIN2Transaction BITCOIN2TxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    BITCOIN2Transaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = BITCOIN2Transaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = BITCOIN2Transaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t BITCOIN2TxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // bitcoin2
    BITCOIN2DGP bitcoin2DGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setBITCOIN2Schedule(bitcoin2DGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = bitcoin2DGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = bitcoin2DGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = bitcoin2DGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // bitcoin2
...
///////////////////////////////////////////////////////////////////////////////////////// bitcoin2
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            BITCOIN2TxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractBITCOIN2TX resultConvertBITCOIN2TX;
            if(!convert.extractionBITCOIN2Transactions(resultConvertBITCOIN2TX)){
            ...
            if(!CheckMinGasPrice(resultConvertBITCOIN2TX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertBITCOIN2TX.first, blockGasLimit, pindex->pprev);
            ...
            for(BITCOIN2Transaction& qtx : resultConvertBITCOIN2TX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertBITCOIN2TX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertBITCOIN2TX.first[k].from(),
                        resultConvertBITCOIN2TX.first[k].to(),
                        ...
                        resultConvertBITCOIN2TX.first[k].getNVout()
                    });
                }
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // bitcoin2
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // bitcoin2
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // bitcoin2
            globalState->setRootUTXO(oldHashUTXORoot); // bitcoin2
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // bitcoin2
        uint32_t sizeBlockDGP = bitcoin2DGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for BITCOIN2PoS we need to sync this after every block to ensure txdb is populated for
