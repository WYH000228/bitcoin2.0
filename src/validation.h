...
#include <bitcoin2/storageresults.h>
...
using ExtractBITCOIN2TX = std::pair<std::vector<BITCOIN2Transaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // bitcoin2
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashBITCOIN2Root,
    const dev::h256& oldHashStateRoot, const std::vector<BITCOIN2Transaction>& transactions);
...
class BITCOIN2TxConverter{

public:

    BITCOIN2TxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionBITCOIN2Transactions(ExtractBITCOIN2TX& bitcoin2Tx);
...
private:
    ...
    BITCOIN2Transaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<BITCOIN2Transaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
