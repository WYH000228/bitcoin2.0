 ...
 */
static const char* BITCOIN_PID_FILENAME = "bitcoin2d.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cryptofugu/bitcoin2chain-core>";
    const std::string URL_WEBSITE = "<https://bitcoin2chain.org>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateBITCOIN2");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path bitcoin2StateDir = GetDataDir() / "stateBITCOIN2";
                bool fStatus = fs::exists(bitcoin2StateDir);
                const std::string dirBITCOIN2(bitcoin2StateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsBITCOIN2state = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<BITCOIN2State>(new BITCOIN2State(dev::u256(0), BITCOIN2State::openDB(dirBITCOIN2, hashDB, dev::WithExisting::Trust), dirBITCOIN2, existsBITCOIN2state));
                dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::bitcoin2MainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(bitcoin2StateDir.string()));
                if (fReset) {
