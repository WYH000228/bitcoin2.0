...
#include <qt/bitcoin2versionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    bitcoin2VersionChecker = new BITCOIN2VersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BITCOIN2address"));
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history"), tr("&Transactions"), this);//BITCOIN2_LINE
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BITCOIN2addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BITCOIN2addresses"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BITCOIN2command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && bitcoin2VersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(BITCOIN2_RELEASES, BITCOIN2_RELEASES);
            QString message(tr("New version of BITCOIN2wallet is available on the BITCOIN2source code repository: %1. It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to BITCOIN2network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("BTC2"); // default title
