...
bool ToHash160(const std::string& strBITCOIN2Address, std::string& strHash160)
{
    CTxDestination bitcoin2Address = DecodeDestination(strBITCOIN2Address);
    if(!IsValidDestination(bitcoin2Address))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&bitcoin2Address);
    if(keyid){
...
bool ToBITCOIN2Address(const std::string& strHash160, std::string& strBITCOIN2Address)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination bitcoin2Address = keyid;
    if(IsValidDestination(bitcoin2Address)){
        strBITCOIN2Address = EncodeDestination(bitcoin2Address);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBITCOIN2Address(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBITCOIN2Address(tokenEvent.receiver, tokenEvent.receiver);
