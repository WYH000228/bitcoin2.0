...
#define paternVersion "bitcoin2-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

BITCOIN2VersionChecker::BITCOIN2VersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

BITCOIN2VersionChecker::~BITCOIN2VersionChecker()
{

}

bool BITCOIN2VersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> BITCOIN2VersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(BITCOIN2_RELEASES)));
    QEventLoop event;
...
Version BITCOIN2VersionChecker::getMaxReleaseVersion()
{
